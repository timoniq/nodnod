...
def test_create_result_node_success():
    result_node = create_result_node(fntypes.Result[int, ValueError])
    ...
    assert result_node.__type__ == fntypes.Result[int, ValueError]


def test_create_result_node_different_types():
    result_node1 = create_result_node(fntypes.Result[str, RuntimeError])
    result_node2 = create_result_node(fntypes.Result[list, TypeError])
...
def test_create_result_node_no_type_args():
    with pytest.raises(NodeBuildError) as exc_info:
        create_result_node(fntypes.Result)  # type: ignore

    assert "Result must have specified type arguments" in str(exc_info.value)


def test_create_result_node_caching():
    result_node1 = create_result_node(fntypes.Result[int, ValueError])
    result_node2 = create_result_node(fntypes.Result[int, ValueError])
...
def test_create_result_node_custom_error():
    ...
    result_node = create_result_node(fntypes.Result[str, CustomError])
